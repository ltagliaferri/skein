...
How a published station (the `skein station` read surface on :9001) gets its
...
A station's identity and presentation come from `.skein-station/stationfile.json` —
...
`stationfile.name` wins; `SKEIN_STATION_NAME` is a **bootstrap that supplies the
