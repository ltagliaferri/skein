...
def resolve_to_hash(address_str: str, store, *, local_authority: Optional[str] = None) -> str:
    ...
    """
    def _alias_or_invalid() -> str:
        # Not a resolvable content-hash address. The freshly-migrated corpus still
        # carries human ids in thread endpoints and pasted refs; resolve those
        # through the alias table before declaring the address malformed.
    ...
    try:
        ...
        return _alias_or_invalid()

    folio = parsed.folio

    # The hardened ``skein.address`` grammar this module retargets onto (§3) adds a
    # bare-slug REF production — ``<slug>``, ``ref::<slug>``, ``<name>::ref::<slug>``,
    # ``web::auth::ref::<slug>`` — resolved via a WORKBENCH lineage head
    # (``head_of_slug``). The station's ``skein_next.address`` predecessor had no such
    # production: those forms raised ``AddressError`` and fell through to the alias
    # table. A station holds immutable multi-author content with NO local lineage, so
    # it has no folio heads, and station reads must never invoke workbench head logic
    # (Risk-3). A parsed ``Ref`` is therefore routed exactly as ``skein_next`` routed
    # it — a legacy human id resolved through the alias table, else invalid — never a
    # ``head_of_slug`` lookup. (This branch precedes the ``web`` block below, whose
    # ``folio.is_short`` touch assumes a ``Hash`` folio.)
    if isinstance(folio, address.Ref):
        return _alias_or_invalid()
