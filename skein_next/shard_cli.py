...
The shard *engine* (:mod:`skein_next.shard`) is pure local git + a
...
content-hash :class:`~skein_next.station.Station` directly — no server.
...
- ``tender`` / merge's auto-tender: POST /folios → :meth:`Station.post`, with the
  ...
  envelope (:mod:`skein_next.tender`) since the ``folios`` table has no metadata
  column.
