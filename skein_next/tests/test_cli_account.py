...
def _run(tmp_path, *args):
    return CliRunner().invoke(cli, ["--data-dir", str(tmp_path / ".skein-next"), *args])


def _open(tmp_path):
    return Station(tmp_path / ".skein-next")
...
def test_rotate_operator_atomic(tmp_path):  # D9
    ...
    with _open(tmp_path) as st:
        ...
        from skein_next.authorization import Principal, can_write
...
def test_read_app_starts_without_operator_under_require_signed(tmp_path, monkeypatch):  # D17
    ...
    from skein_next.web import app as web_app
    monkeypatch.setenv("SKEIN_NEXT_DATA_DIR", str(tmp_path / ".skein-next"))
    monkeypatch.setenv("SKEIN_NEXT_REQUIRE_SIGNED", "1")
    monkeypatch.setenv("SKEIN_NEXT_PROJECT", "interskein")
    Station(tmp_path / ".skein-next").close()  # empty corpus, no operator
...
def test_ingress_startup_logs_operator_status(tmp_path, monkeypatch, caplog):  # D18
    ...
    ingress = _ingress_app(tmp_path, monkeypatch, require_signed=True)
    import logging
    with caplog.at_level(logging.INFO, logger="skein_next.ingress"):
        ingress.create_app()
