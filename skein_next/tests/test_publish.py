...
from skein_next.ingress import ingest, create_app
from skein_next import publish as pub_mod
from skein_next.publish import collect_publish_set, PublishError, post_batch, canonical_instance
...
def test_ingress_route_round_trip_and_guards(client, tmp_path, monkeypatch):
    ...
    _seed_site(client)
    batch = wire.build_batch(*collect_publish_set(client, site="specs"))
    monkeypatch.setenv("SKEIN_NEXT_DATA_DIR", str(tmp_path / "routed" / ".skein-next"))
