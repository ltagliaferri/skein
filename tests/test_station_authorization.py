...
Re-homed from skein_next/tests/test_authorization.py (station re-home Stage 3), over
``skein.authorization`` + the ``StationStore`` binding accessors. The authorization half
is orthogonal to signing: identity is the verified cert (issuer, subject) pair, never
created_by/weaver. Units run against an in-memory fake BindingStore; the storage cells run
against the real account_bindings + binding_events tables.
"""
...
def test_bootstrap_operator_concurrent_race_yields_exactly_one_operator(tmp_path):  # finding-6(b)
    ...
    StationStore(data_dir).close()  # materialize so racers don't birth in parallel

    def attempt(i):
        st = StationStore(data_dir, check_same_thread=False)
        try:
    ...
    check = StationStore(data_dir, check_same_thread=False)
    try:
