...
Re-homed from skein_next/tests/test_invite_redeem_hardening.py (station re-home Stage 5),
over the re-homed ``skein.ingress`` + ``skein.redeem`` + ``StationStore`` (all landed
Stage 3). The DROP fat-client ``_seed_folio`` (``Station.create_site``/``post``) is
rebuilt from field dicts via ``tests/station_publish_helpers`` — the same stand-in the
Stage-3 ingress round-trip suites use; the two load-bearing properties below are otherwise
byte-faithful and must still FIRE.
...
def _seed_folio(tmp_path):
    """A single valid wire folio dict for a concurrent publish.

    skein_next authored this through ``Station.create_site``/``post`` (DROP); the
    station-re-home builds it directly from canonical fields — the stand-in the
    Stage-3 ingress suites use — and serializes it to the wire shape ``ingest`` reads."""
    return wire.folio_to_wire(h.folio("finding", "T", "body", "2026-01-01T00:00:00+00:00"))
