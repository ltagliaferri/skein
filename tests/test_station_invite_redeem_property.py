...
Re-homed from skein_next/tests/test_invite_redeem_property.py (station re-home Stage 5),
over the re-homed ``skein.redeem`` + ``skein.sign`` + the ``StationStore`` invite accessors
(all landed Stage 3). Pure import retarget: this suite drives the store + redeem + sign
modules directly (no authoring verbs, no HTTP), so nothing but the import roots changes —
the meql finding-6 security invariants must still FIRE after the move.

The enumerated cases in ``test_station_invite_redeem.py`` cover the named invariants; these
...
# --- fakes (mirror test_station_invite_redeem.py) ---------------------------
...
def test_redeem_state_machine_invariants(tmp_path_factory, ops):
    ...
    inst = tmp_path_factory.mktemp("prop") / ".skein-next"
...
def test_cheap_reject_never_reaches_crypto(tmp_path_factory, state):
    ...
    inst = tmp_path_factory.mktemp("cheap") / ".skein-next"
    station = Station(inst)
    token = "tok-cheap"
    try:
        if state == "unknown":
