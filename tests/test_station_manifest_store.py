...
Re-homed from skein_next/tests/test_manifest_store.py (station re-home Stage 3), now
over ``StationStore``'s federation accessors on the shared station schema. ONE manifest
store (triple PK) + ONE attribution table over folios AND threads. The B50
two-signers-same-set property survives on the (root, issuer, subject) triple PK (ST3).
"""
