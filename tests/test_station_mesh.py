...
Re-homed from skein_next/tests/test_mesh.py (station re-home Stage 5), over the re-homed
``skein.mesh.client`` + ``skein.sign`` + ``skein.web.app``. The corpus is seeded via
``tests/station_read_helpers.StationBuilder`` (the DROP fat-client ``Station.create_site``
/``post`` verbs) and the legacy-id alias via the ``StationStore.set_alias`` accessor. This
suite also PINS the one behavior adaptation the mesh re-home carries: ``_pin_check`` now
routes over the hardened ``skein.address`` grammar, whose bare-slug REF production would
otherwise crash the pin — ``test_legacy_id_address_is_unpinned`` +
``test_pin_check_full_prefix_and_mismatch`` prove the byte-faithful unpinnable fallback.

The HTTP wire is backed by a TestClient of the station app via a `requests.get` shim,
so these exercise the real resolve+envelope path end to end. The signed verdict branches
monkeypatch `sign.verify_wire_folio` (the crypto itself is the signing suite's job; here
we pin the client's fork-F mapping over its result).
"""
...
def test_pin_check_full_prefix_and_mismatch():
    ...
    # bare alias / legacy id -> unpinnable (skein.address parses it as a REF; the
    # re-home routes that to the byte-faithful unpinnable fallback, never a crash)
