...
Re-homed from skein_next/tests/test_mesh_mcp.py (station re-home Stage 5), over the
re-homed ``skein.mesh`` + ``skein.web.app``; the corpus is seeded via
``tests/station_read_helpers.StationBuilder`` (the DROP fat-client authoring verbs).
