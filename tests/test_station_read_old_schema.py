...
predates the manifest/authorization tables). The hazard corpus is a station store
(``versions``/``threads``/``station_slugs``) with NONE of the federation sidecars
(``manifests``, ``constituent_attribution``, ``verify_cache``, ``account_bindings``,
``binding_events``). Opening that corpus with the read app and asking for a folio
verdict must degrade to UNSIGNED, never raise
...
def _assert_serves_unsigned(data_dir, h):
    ...
    ro = StationStore(data_dir, read_only=True)
    try:
        row = ro.get_folio(h)
...
def test_covered_folio_with_account_bindings_table_absent_degrades(tmp_path, monkeypatch):
    ...
    from skein.station_store import bundle_hash_for
...
def _open_rw_then_drop(data_dir, table):
    ...
    store = StationStore(data_dir)  # read_write: migration ran, table exists
...
def test_read_only_store_account_bindings_absent_degrades(tmp_path):
    ...
    with StationStore(tmp_path / ".skein-next", read_only=True) as ro:
