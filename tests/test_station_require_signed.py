...
def test_on_covered_bound_constituent_accepts(instance):  # RS7
    _bind(instance)
    ack = ingest(instance, _signed_batch(), verifier=_ok_verifier, require_signed=True)
...
def test_on_missing_account_bindings_table_raises_not_silent_reject(instance):
...
def test_off_never_consults_bindings_or_manifest(instance):  # RS9
...
def test_attribution_is_manifest_signer_not_created_by_or_weaver(instance):  # RS11
    _bind(instance)
...
def test_ingest_default_bindings_is_station_store(instance):  # RS12
    _bind(instance)
...
def test_old_manifest_replay_cannot_inject_new_leaf(instance):  # RS14
    _bind(instance)
...
def test_slug_cross_author_overwrite_is_last_write_wins_v0(instance):  # RS19
    _bind(instance)
...
def test_ingress_exposes_no_binding_mutation_route(monkeypatch):  # RS17
...
def test_off_then_on_existing_body_acquires_attribution(instance):  # RS20
...
def test_off_then_on_unverified_manifest_stays_unattributed(instance):  # RS20 negative
    off_batch = _unsigned_batch()
    ingest(instance, off_batch, require_signed=False)
    finding = _finding_hash(off_batch)
    _bind(instance)
...
def test_on_slug_write_gated_by_site_folio_membership(instance):  # RS18
    _bind(instance)
    folios, threads, slugs = h.specs_set()
    batch = wire.build_batch(folios, threads, slugs)
    finding = next(f["content_hash"] for f in batch["folios"] if f["type"] == "finding")
...
def test_subset_replay_of_own_manifest_accepted(instance):  # C41
    _bind(instance)
    folios, threads, slugs = h.specs_set()
...
def test_revoked_between_publishes_re_gates_per_ingest(instance):  # C40
    _bind(instance)
    folios, threads, slugs = h.specs_set()
    full = wire.build_batch(folios, threads, slugs)
    ms = h.manifest_over(folios, threads)
...
def test_cross_instance_replay_contained_by_per_instance_binding(tmp_path):  # C42
    batch = _signed_batch()
...
def test_reactivation_replay_accepted_on_rebind(instance):  # C44
    _bind(instance)
    folios, threads, slugs = h.specs_set()
    full = wire.build_batch(folios, threads, slugs)
    ms = h.manifest_over(folios, threads)
...
def test_ingress_non_dict_manifest_signature_passes_shape_gate():  # VM11
...
def test_ingress_non_dict_manifest_signature_per_verdict_not_500(instance, bad):  # VM11
    _bind(instance)
    batch = _signed_batch()
...
def test_ingress_absent_manifest_key_is_no_manifest(instance):  # VM11
    _bind(instance)
    batch = _signed_batch()
...
def test_ingress_non_dict_manifest_signature_off_unaffected(instance):  # VM11
    batch = _signed_batch()
...
def test_ingress_malformed_manifest_per_verdict_not_500(instance):  # VM12
    _bind(instance)
    batch = _signed_batch()
...
def test_off_never_invokes_verifier_with_present_manifest(instance):  # FIX 1 (a)
...
def test_off_with_raising_verifier_still_publishes(instance):  # FIX 1 (b)
    batch = _signed_batch()
...
def test_on_malformed_manifest_rejects_every_constituent_identically(instance):  # FIX 2 (a)
    _bind(instance)
...
def test_on_valid_manifest_over_thread_not_endpoints_accepts_dangling(instance):  # FIX 2 (b)
    _bind(instance)
    folios, threads, slugs = h.specs_set()
    batch = wire.build_batch(folios, threads, slugs)
...
def test_on_unbound_signer_rejects_thread_with_bind_reason(instance):  # FIX 2 (c)
