...
Re-homed at station Stage 4. resolve.py retargets ``skein_next.address`` ->
``skein.address`` (the hardened superset, §3); these pins are the parity proof that
the retarget preserves short-hash / web:: / alias / fragment resolution. The
real-store checks build over the Stage-1 ``StationStore`` (its ``create_folio`` +
``find_by_prefix`` + ``resolve_alias`` accessors) instead of ``SkeinNextStore``.
"""
