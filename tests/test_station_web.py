...
def seeded(data_dir):
    ...
    with StationBuilder(data_dir) as st:
        ...
        # Faithful status shape: a self-loop (from_id == to_id == folio), not the
        # skein_next fixture's from_id-less edge the nullable store allowed.
        st.set_status(b, "closed", created_at="2026-01-04T00:00:00Z")
...
def _cover_folio(data_dir, content_hash, *, subject="alice@example.com",
    ...
    store = StationStore(data_dir, check_same_thread=False)
...
def test_proof_missing_not_called_unsigned(seeded, monkeypatch):
    ...
    client = _make_client(seeded["data_dir"], monkeypatch, stationfile={"name": "X"})
    store = StationStore(seeded["data_dir"], check_same_thread=False)
...
def test_integrity_fail_not_called_unsigned(seeded, monkeypatch):
    ...
    client = _make_client(seeded["data_dir"], monkeypatch, stationfile={"name": "X"})
    store = StationStore(seeded["data_dir"], check_same_thread=False)
    # Station folios live in ``versions`` (skein_next's flat ``folios`` table is
    # retired); tamper the body there so it no longer hashes to its content_hash key.
    store.conn.execute(
